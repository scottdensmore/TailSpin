...
$configuredSSLBinding = gwmi -namespace "root/webadministration" -query "select * from SSLBinding where Port=443 and IPAddress='*'";
if ($configuredSSLBinding -eq $null)
{
    $certificateForHttpsBinding = Get-CertificateBySubject -StoreLocation 'LocalMachine' -StoreName 'My' -Subject 'CN=localhost';
    if ($certificateForHttpsBinding -eq $null)
    {
        $AppPoolUser = Get-DefaultAppPoolUserName
        Import-CerCertificate -CertPath (resolve-path ".\certs\root.cer") -StoreLocation 'LocalMachine' -StoreName 'AuthRoot'
        Import-PfxCertificate -CertPath (resolve-path ".\certs\localhost.pfx") -PfxPass 'xyz' -StoreLocation 'LocalMachine' -StoreName 'My' -UserNameThatWillUseIt $AppPoolUser
        Import-PfxCertificate -CertPath (resolve-path ".\certs\localhost.pfx") -PfxPass 'xyz' -StoreLocation 'LocalMachine' -StoreName 'TrustedPeople'

        $certificateForHttpsBinding = Get-CertificateBySubject -StoreLocation 'LocalMachine' -StoreName 'My' -Subject 'CN=localhost';
    }
    $sslBindingClass = [WMIClass]'root\webadministration:SSLBinding';
    $InParams = $sslBindingClass.psbase.GetMethodParameters("Create");
    $InParams.CertificateHash = $certificateForHttpsBinding.Thumbprint;
    $InParams.IPAddress = '*';
    $InParams.Port = 443;
    $InParams.StoreName = 'MY';
    $sslBindingClass.psbase.InvokeMethod("Create", $InParams, $Null);
}

$bindingElementClass = [WMIClass]'root\webadministration:BindingElement';
$newBinding = $bindingElementClass.CreateInstance();
...
$website.Bindings += [System.Management.ManagementBaseObject] $newBinding;
$website.Put();
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUUkl9vhro/Em2QGtAFsmo5u9V
# MsagghXnMIIEhTCCA22gAwIBAgIKYQh3XwAAAAAASjANBgkqhkiG9w0BAQUFADB5
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFOes98Wd
# qqaPppHLlOGSHiW0z2LhMIGsBgorBgEEAYI3AgEMMYGdMIGaoHCAbgBwAGEAdAB0
...
# BgkqhkiG9w0BAQEFAASCAQBbq0jaMOZ5PKW5b5OKLaAQdlYsiSLFI6FZ8dcYULx2
# Gdjjsr1UVeAmFM9XJj5AvJLkfZD9uNcxi1ympKeDG1k0eBI49ovtxrXWPL8CntTE
# whdOn8AcxcOQLpd0g1tRyn/yA/Wh47GE9xBQbC/l3AUoCPZTUEGYkKDiXx/sqn27
# 5DVuyJw+nV6YKw/rO3quGuieTaPTU03hbNl7GxyrxQSShkRNH3bnjD+C6PZ09x5M
# e6nSWuWT//EaaXp5lFXjVMzgIgYYSuKvt89u5UNQ9oKK23I2UVqQNjj0zvjvA7ek
# EHq5lv9QBRmZQXSNdwL6me/kbNDSvtJg/sjxqQ1qjuQjoYICHTCCAhkGCSqGSIb3
...
# KoZIhvcNAQkFMQ8XDTEwMTAyNzE5NTkxNFowIwYJKoZIhvcNAQkEMRYEFGW4YiRZ
# NpP97T5AgbpoBzji9urWMA0GCSqGSIb3DQEBBQUABIIBAEB+FITz5jF2l4pGex6r
# xsqSxt+5Eubz9GyZfaAq5seK2HKaN/fug+5rynohX24FHJOZgnskVgZY6MVBZ2ag
# YdS8ovBZx2zu9sjm5oQpzwg//lWgh0RBVWI64YnUhB9kE6/9nL1JmsBjQPZ8wL/8
# l5345T/PpJoe7ZU2GV+V6C+tzCBOvbQXIZ7d+m4kTXLtd8ER0nOXQPS2FcCSOmX5
# w3QYezPMLnR5QDAt6MsrQJHNYEhH7v8+FnxmmlNvpYRZ/DsCAyGc+MQW3nlGD4Hg
# 8Q1kEPWse9tw6T00WdFPmKkLjHIbmzc1PQeiul/j+yWVDgN/MaKyOPZPqGBWJoBk
# Ed4=
