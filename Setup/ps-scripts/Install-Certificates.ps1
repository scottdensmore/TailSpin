. ".\PS-Scripts\Cmds-Certificates.ps1"

$AppPoolUser = Get-DefaultAppPoolUserName

if ((Check-CertificateBySubject -StoreLocation 'LocalMachine' -StoreName 'My' -Subject 'CN=adatum') -ne $true)
{
    Import-PfxCertificate -CertPath (resolve-path ".\certs\adatum.pfx") -PfxPass 'Passw0rd!' -StoreLocation 'LocalMachine' -StoreName 'My' -UserNameThatWillUseIt $AppPoolUser
}

if ((Check-CertificateBySubject -StoreLocation 'LocalMachine' -StoreName 'TrustedPeople' -Subject 'CN=adatum') -ne $true)
{
    Import-PfxCertificate -CertPath (resolve-path ".\certs\adatum.pfx") -PfxPass 'Passw0rd!' -StoreLocation 'LocalMachine' -StoreName 'TrustedPeople'
}

if ((Check-CertificateBySubject -StoreLocation 'LocalMachine' -StoreName 'My' -Subject 'CN=tailspin') -ne $true)
{
    Import-PfxCertificate -CertPath (resolve-path ".\certs\tailspin.pfx") -PfxPass 'Passw0rd!' -StoreLocation 'LocalMachine' -StoreName 'My' -UserNameThatWillUseIt $AppPoolUser
}

if ((Check-CertificateBySubject -StoreLocation 'LocalMachine' -StoreName 'TrustedPeople' -Subject 'CN=tailspin') -ne $true)
{
    Import-PfxCertificate -CertPath (resolve-path ".\certs\tailspin.pfx") -PfxPass 'Passw0rd!' -StoreLocation 'LocalMachine' -StoreName 'TrustedPeople'
}

if ((Check-CertificateBySubject -StoreLocation 'LocalMachine' -StoreName 'My' -Subject 'CN=fabrikam') -ne $true)
{
    Import-PfxCertificate -CertPath (resolve-path ".\certs\fabrikam.pfx") -PfxPass 'Passw0rd!' -StoreLocation 'LocalMachine' -StoreName 'My' -UserNameThatWillUseIt $AppPoolUser
}

if ((Check-CertificateBySubject -StoreLocation 'LocalMachine' -StoreName 'TrustedPeople' -Subject 'CN=fabrikam') -ne $true)
{
    Import-PfxCertificate -CertPath (resolve-path ".\certs\fabrikam.pfx") -PfxPass 'Passw0rd!' -StoreLocation 'LocalMachine' -StoreName 'TrustedPeople'
}

# SIG # Begin signature block
# MIIbXgYJKoZIhvcNAQcCoIIbTzCCG0sCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUU4RtmnBw7cDG5pnsUUssp/Nh
# Z4KgghXnMIIEhTCCA22gAwIBAgIKYQh3XwAAAAAASjANBgkqhkiG9w0BAQUFADB5
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFMUYc5yU
# IabX0NxOSKn+KrITnmpQMIGsBgorBgEEAYI3AgEMMYGdMIGaoHCAbgBwAGEAdAB0
...
# BgkqhkiG9w0BAQEFAASCAQAGM7k84pEyxE/KhB44/gTQXbaSFzkglu1aMA8Jm2md
# +5pliZccKl9iz8wCXEQDNlDa8V3R27E+0iY4k+y8jJ5BaS2hJCwOcvS7J/yAKQl0
# e0rNibX/LPen2SRLcVq0qoNjlqf7JLPeCQeOaUd8zjSXxkegybZd9UTCQ1V1M7UW
# VqLZgkKkx0TzXeXh9ySZHOVrzETRD9yZ70snnSOEdw1CktuGv/hAGZERNMu29Qa/
# oRrC+jN76cCv+lAyEA4xHWw1+08L4o3++1kjJZ4VPxxFGJRne1ltPFchENEiwNvi
# ekmPAKUGzuRIzgehNaGhmQ910w0vO0ptzq3f/357Bnb4oYICHTCCAhkGCSqGSIb3
...
# KoZIhvcNAQkFMQ8XDTEwMTAyNzE5NTkxNFowIwYJKoZIhvcNAQkEMRYEFEL67how
# QLBs/U/A8m3n33EyxJbsMA0GCSqGSIb3DQEBBQUABIIBAJVV/PV6bw1qmLZdZU5p
# ZmwrzBo5cvBIiM/j75f14jTaYQSpqGGcZPrr7qbXCODm6hlIgzR81htEwWjO+1uM
# rEB+OfoHeV7kSR/7hHNAwJZjETSObWpwRc1CImtJY9145REUXvpj1OoU7gtHUPXg
# b0ixHmPMAI0C/SIuLIBW54rRWdUfA81jdgTjCO29Trahe+/3Moc1Q4wIZH9fHWUT
# nuyy7WI4yahBobJ0iG6IumR/l/XuvdAwmEPV9n9CtjsZx4AWfOPTLwSTRXxb0d5H
# vM5Mnb8JMojGh3EfanEiFF58V2KuYueUjAEUryyJ97/q7Z+qLyTNG62HHJK3X4J7
# ovQ=
