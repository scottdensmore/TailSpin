function Check-CertificateBySubject($StoreLocation, $StoreName, $Subject)
{
    $cert = Get-CertificateBySubject -StoreLocation $StoreLocation -StoreName $StoreName -Subject $Subject
...
function Get-CertificateBySubject($StoreLocation, $StoreName, $Subject)
{
    $certificatePath = "cert:\$StoreLocation\$StoreName\";

    if ((Test-Path -path $certificatePath) -ne $True) 
	{
		return $null;
	}

    return @(dir $certificatePath | ? { $_.Subject -like "$Subject" })
}

function Import-PfxCertificate($CertPath, $PfxPass, $StoreLocation, $StoreName, $UserNameThatWillUseIt) 
{
    if ((Test-Path -path $CertPath) -ne $True) 
	{
		return $FALSE;
	}
    ...
    if ($UserNameThatWillUseIt -ne $null)
    {
        $rsa = [System.Security.Cryptography.RSACryptoServiceProvider] $pfx.PrivateKey;

        if ($rsa -ne $null)
        {
            $keyFileName = $rsa.CspKeyContainerInfo.UniqueKeyContainerName
           
            $MachineKeysPath = [System.Environment]::GetFolderPath("CommonApplicationdata") + '\Microsoft\Crypto\RSA\MachineKeys';
            $fileName = (dir $MachineKeysPath) | ? {$_.Name -eq $keyFileName};
            if ($fileName -eq $null)
            {
                throw 'Can find private key file'
            }
            
            $file = resolve-path "$MachineKeysPath\$fileName"
            $acl = Get-Acl $file
            $permission = "$UserNameThatWillUseIt", 'FullControl', 'Allow'
        ...
        {
            throw 'Can read certificate''s private key'
        }
    }
}

function Import-CerCertificate($CertPath, $StoreLocation, $StoreName) 
{
    if ((Test-Path -path $CertPath) -ne $True) 
	{
		return $FALSE;
...
function Get-DefaultAppPoolUserName()
{
    $windowsVersion = [environment]::osversion.Version.ToString()
    if ($windowsVersion.StartsWith('6.1'))
    {
        return 'IIS_IUSRS'
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUh6tCns9Dzb3P2ahNCfeB8xU8
# S/mgghXnMIIEhTCCA22gAwIBAgIKYQh3XwAAAAAASjANBgkqhkiG9w0BAQUFADB5
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFMYXi+Gv
# pomIoSrDFTAtjVmKyIC8MIGsBgorBgEEAYI3AgEMMYGdMIGaoHCAbgBwAGEAdAB0
...
# BgkqhkiG9w0BAQEFAASCAQBJgekfexAZlwY1/ljD9E46LVB2NAh06qWPzJpsauLE
# kzGjsNn/3cbCkIcrcMjFfbpZYY8LWtr+Vd3MkLRupYzlhEsh8FRIK7h+CGoqn6M/
# 9rog+J5jeeWiqt4k+W/Zp69LjfIJ8LlNIlrKMMmzwu1Azl/1YMoKmkSACZo+GrFs
# 2b7g+zZznRSeWq95dyyotGA8O6fZtHaUkovTeaCRAVPcppawGXTp8tYTVv2KEosH
# J3uFGbL8T8muKLfaL0Dp7zFpVhLzXHnHNdcGWAga7EviY7pd1Zt57R3ypv3AnQO5
# SdQBQJZP0GoK39yQQYOlbNDHPE2p+x/4DgKInjsESXu9oYICHTCCAhkGCSqGSIb3
...
# KoZIhvcNAQkFMQ8XDTEwMTAyNzE5NTkxM1owIwYJKoZIhvcNAQkEMRYEFEQL4kqk
# Ow9lT2UWfFPBUuLACD7eMA0GCSqGSIb3DQEBBQUABIIBACnoLh43F2I8cqNgh7hx
# wWVF1YYhLEtY8PHccJ0FVnOYr+TfByt4rE2m7N7a1FboD58HKpEZpDJzvLv0UdaZ
# GF5LNLWS3j4GiwaYzufyZMxg/Tr4mjXJU26zZDXamibYPD/U7HpN5IMYyvokhU6e
# +x8xq9w264R0QrLkuFMY8OqBAcGPTWC5xQDHM03dxyo1zzsWXVW9t2YttmE5p4FE
# 0teFcTQLcOsn5gG7JW8nk6omCZY3UTsa7EyT087FtGZrTfLy7GAwbG9Lf/U5uBnV
# NwIB5GzWW1/DUIwfylwhwkNm4Zv4CxuBbvRqoy6RzAjRRgW3H5MJfvPT57XxZFX9
# Am8=
